...
export default function CalendarPage() {
  ...
  // クライアントサイドでのみ日付を設定
  useEffect(() => {
    setSelectedDate(new Date());  // 初期選択日付を現在の日付に設定
  }, []);
  ...
  const handleAddReservation = () => {
    ...
    // 次のIDを決定する（ここではローカルで生成）
    // const nextId = reservations.length > 0 ? Math.max(...reservations.map(res => res.id)) + 1 : 1;

    const newReservation: Reservation = {
      id: 1, // 自動インクリメントされたIDを使用
